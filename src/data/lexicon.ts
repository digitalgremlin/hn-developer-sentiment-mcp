// Sentiment lexicon: VADER valence (word → mean rating in [-4, 4]), expanded from the
// canonical VADER lexicon (Hutto, C.J. & Gilbert, E.E. 2014; MIT-licensed):
// https://github.com/cjhutto/vaderSentiment  (vaderSentiment/vader_lexicon.txt).
// Filtered to word-shaped tokens the scorer's tokenizer can match (/^[a-z']+$/);
// emoticon/non-word entries are handled separately by EMOJI below. A small set of
// dev-context terms is overlaid on top of the VADER values (see git history).
// This file is generated data — the scorer (src/sentiment.ts) is unchanged.
...
export const INTENSIFIERS: Record<string, number> = { really: 0.3, very: 0.3, so: 0.25, extremely: 0.4, super: 0.3 };
export const NEGATIONS = new Set(["not", "no", "never", "n't", "without", "cannot", "cant"]);
export const EMOJI: Record<string, number> = { "🙂": 1.0, "👍": 1.5, "🔥": 1.2, "😤": -1.2, "👎": -1.5, ":)": 1.0, ":(": -1.0 };
